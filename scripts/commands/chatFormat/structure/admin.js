...
export const admin = (player, command) => {
  if (command[3] == undefined) {
    Hutao.World.showHelp(player,
      [`structure admin §dset`, Hutao.Player.getLanguage(player).commandChatFormatStructureAdmin],
      [`structure admin §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[3] == "set") {
    if (command[4] == undefined) {
      Hutao.World.showHelp(player,
        [`admin set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatStructureAdmin]
      )
    } else {
      ...
      config.data.chatFormat.structure.admin = Hutao.World.getNormalCommand(player, 4);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).admin} §7▶ §r${setting.default.data.chatFormat.structure.admin}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
