...
export const owner = (player, command) => {
  if (command[3] == undefined) {
    Hutao.World.showHelp(player,
      [`structure owner §dset`, Hutao.Player.getLanguage(player).commandChatFormatStructureOwner],
      [`structure owner §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[3] == "set") {
    if (command[4] == undefined) {
      Hutao.World.showHelp(player,
        [`owner set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatStructureOwner]
      )
    } else {
      ...
      config.data.chatFormat.structure.owner = Hutao.World.getNormalCommand(player, 4);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).owner} §7▶ §r${setting.default.data.chatFormat.structure.owner}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
