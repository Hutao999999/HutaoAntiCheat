...
export const member = (player, command) => {
  if (command[3] == undefined) {
    Hutao.World.showHelp(player,
      [`structure member §dset`, Hutao.Player.getLanguage(player).commandChatFormatStructureMember],
      [`structure member §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[3] == "set") {
    if (command[4] == undefined) {
      Hutao.World.showHelp(player,
        [`member set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatStructureMember]
      )
    } else {
      ...
      config.data.chatFormat.structure.member = Hutao.World.getNormalCommand(player, 4);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).member} §7▶ §r${setting.default.data.chatFormat.structure.member}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
