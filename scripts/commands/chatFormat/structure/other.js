  ...
  if (command[3] == undefined) {
    ...
    if (command[4] == "set") {
      if (command[5] == undefined) {
        ...
        config.data.chatFormat.structure.custom[currentPlayer.id] = Hutao.World.getNormalCommand(player, 5).trim()
      ...
      if (!Object.keys(setting.default.data.chatFormat.structure.custom).includes(currentPlayer.id)) return Hutao.World.wrong(player, Hutao.Player.getLanguage(player).thePlayerHasNoStructure)

      let config = Hutao.Database.get("db")

      delete config.data.chatFormat.structure.custom[currentPlayer.id]
      ...
      Hutao.World.log([
        ...
        ``,
        `§e${currentPlayer.name} §7▶ §r${setting.default.data.chatFormat.structure.custom[currentPlayer.id]}`,
        `§7=========================`
      ].join("\n§r"), player)
