...
export const spectator = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`gamemode spectator §dset`, Hutao.Player.getLanguage(player).commandChatFormatGamemodeSpectator],
      [`gamemode spectator §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`spectator set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatGamemodeSpectator]
      )
    } else {
      ...
      config.data.chatFormat.format.gamemode.spectator = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).spectator} §7▶ §r${setting.default.data.chatFormat.format.gamemode.spectator}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
