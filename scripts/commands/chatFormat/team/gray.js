...
export const gray = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`team gray §dset`, Hutao.Player.getLanguage(player).commandChatFormatTeamGray],
      [`team gray §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`gray set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatTeamGray]
      )
    } else {
      ...
      config.data.chatFormat.format.team.gray = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).gray} §7▶ §r${setting.default.data.chatFormat.format.team.gray}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
