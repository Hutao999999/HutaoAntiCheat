...
export const rotation = (player, command) => {
  if (command[3] == undefined) {
    Hutao.World.showHelp(player,
      [`format rotation §dset`, Hutao.Player.getLanguage(player).commandChatFormatRotation],
      [`format rotation §dopen`, Hutao.Player.getLanguage(player).commandOpenChatFormatFormatRotationMenu],
      [`format rotation §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[3] == "set") {
    if (command[4] == undefined) {
      Hutao.World.showHelp(player,
        [`rotation set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatRotation]
      )
    } else {
      ...
      config.data.chatFormat.format.rotation = Hutao.World.getNormalCommand(player, 4);
    ...
    new ChatFormat().rotation(player)

    Hutao.World.success(player, Hutao.Player.getLanguage(player).closeTheChatBarToOpenChatFormatFormatRotationMenu)
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).rotation} §7▶ §r${setting.default.data.chatFormat.format.rotation}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
