...
export const owner = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`status owner §dset`, Hutao.Player.getLanguage(player).commandChatFormatStatusOwner],
      [`status owner §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`owner set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatStatusOwner]
      )
    } else {
      ...
      config.data.chatFormat.format.status.owner = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).owner} §7▶ §r${setting.default.data.chatFormat.format.status.owner}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
