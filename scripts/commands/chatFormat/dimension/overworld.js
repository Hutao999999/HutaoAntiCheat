...
export const overworld = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`dimension overworld §dset`, Hutao.Player.getLanguage(player).commandChatFormatDimensionOverworld],
      [`dimension overworld §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`overworld set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatDimensionOverworld]
      )
    } else {
      ...
      config.data.chatFormat.format.dimension.overworld = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).overworld} §7▶ §r${setting.default.data.chatFormat.format.dimension.overworld}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
