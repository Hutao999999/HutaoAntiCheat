...
export const other = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`dimension other §dset`, Hutao.Player.getLanguage(player).commandChatFormatDimensionOther],
      [`dimension other §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`other set §d<string>`, Hutao.Player.getLanguage(player).commandChatFormatDimensionOther]
      )
    } else {
      ...
      config.data.chatFormat.format.dimension.other = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).other} §7▶ §r${setting.default.data.chatFormat.format.dimension.other}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
