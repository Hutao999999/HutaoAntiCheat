...
export const killauraA = (player, command) => {
  if (command[3] == undefined) {
    if (command[4]) return Hutao.World.wrongCommand(player, command, 4)

    Hutao.World.showHelp(player,
      [`killaura a §dflags`, Hutao.Player.getLanguage(player).commandFlags],
      [`killaura a §dopen`, Hutao.Player.getLanguage(player).commandOpenKillauraAMenu],
      [`killaura a §dpunishment`, Hutao.Player.getLanguage(player).commandPunishment],
      [`killaura a §dreset`, Hutao.Player.getLanguage(player).commandReset],
      [`killaura a §dstate`, Hutao.Player.getLanguage(player).commandState],
      [`killaura a §dview`, Hutao.Player.getLanguage(player).commandView]
    )
    ...
    if (command[4] != undefined) {
      ...
      if (command[4] == "true") {
        config.data.antiCheat.killauraA.state = true
      } else if (command[4] == "false") {
        config.data.antiCheat.killauraA.state = false
      } else {
    ...
    new AntiCheat().killauraA(player)

    Hutao.World.success(player, Hutao.Player.getLanguage(player).closeTheChatBarToOpenKillauraAMenu)
    ...
    if (command[4] != undefined) {
      ...
      config.data.antiCheat.killauraA.flags = Math.floor(Number(command[4]))
    ...
    Hutao.World.log([
      `§7=========================`,
      `§e${Hutao.Player.getLanguage(player).project} §7▶ §6Killaura/A`,
      ``,
    ...
    config.data.antiCheat.killauraA = configCache.data.antiCheat.killauraA
