...
export const aqua = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`team aqua §dset`, Hutao.Player.getLanguage(player).commandNameTagTeamAqua],
      [`team aqua §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`aqua set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagTeamAqua]
      )
    } else {
      ...
      config.data.nameTag.format.team.aqua = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).aqua} §7▶ §r${setting.default.data.nameTag.format.team.aqua}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
