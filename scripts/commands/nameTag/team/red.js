...
export const red = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`team red §dset`, Hutao.Player.getLanguage(player).commandNameTagTeamRed],
      [`team red §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`red set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagTeamRed]
      )
    } else {
      ...
      config.data.nameTag.format.team.red = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).red} §7▶ §r${setting.default.data.nameTag.format.team.red}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
