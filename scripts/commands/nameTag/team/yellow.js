...
export const yellow = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`team yellow §dset`, Hutao.Player.getLanguage(player).commandNameTagTeamYellow],
      [`team yellow §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`yellow set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagTeamYellow]
      )
    } else {
      ...
      config.data.nameTag.format.team.yellow = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).yellow} §7▶ §r${setting.default.data.nameTag.format.team.yellow}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
