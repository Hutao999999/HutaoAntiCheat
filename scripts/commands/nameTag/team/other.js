...
export const other = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`team other §dset`, Hutao.Player.getLanguage(player).commandNameTagTeamOther],
      [`team other §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`other set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagTeamOther]
      )
    } else {
      ...
      config.data.nameTag.format.team.other = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).other} §7▶ §r${setting.default.data.nameTag.format.team.other}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
