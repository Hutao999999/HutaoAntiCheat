...
export const green = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`team green §dset`, Hutao.Player.getLanguage(player).commandNameTagTeamGreen],
      [`team green §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`green set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagTeamGreen]
      )
    } else {
      ...
      config.data.nameTag.format.team.green = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).green} §7▶ §r${setting.default.data.nameTag.format.team.green}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
