...
export const builder = (player, command) => {
  if (command[3] == undefined) {
    Hutao.World.showHelp(player,
      [`structure builder §dset`, Hutao.Player.getLanguage(player).commandNameTagStructureBuilder],
      [`structure builder §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[3] == "set") {
    if (command[4] == undefined) {
      Hutao.World.showHelp(player,
        [`builder set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagStructureBuilder]
      )
    } else {
      ...
      config.data.nameTag.structure.builder = Hutao.World.getNormalCommand(player, 4);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).builder} §7▶ §r${setting.default.data.nameTag.structure.builder}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
