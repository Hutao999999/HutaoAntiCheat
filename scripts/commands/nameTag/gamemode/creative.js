...
export const creative = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`gamemode creative §dset`, Hutao.Player.getLanguage(player).commandNameTagGamemodeCreative],
      [`gamemode creative §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`creative set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagGamemodeCreative]
      )
    } else {
      ...
      config.data.nameTag.format.gamemode.creative = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).creative} §7▶ §r${setting.default.data.nameTag.format.gamemode.creative}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
