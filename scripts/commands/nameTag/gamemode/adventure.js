...
export const adventure = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`gamemode adventure §dset`, Hutao.Player.getLanguage(player).commandNameTagGamemodeAdventure],
      [`gamemode adventure §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`adventure set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagGamemodeAdventure]
      )
    } else {
      ...
      config.data.nameTag.format.gamemode.adventure = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).adventure} §7▶ §r${setting.default.data.nameTag.format.gamemode.adventure}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
