...
export const state = (player, command) => {
  if (command[2] == undefined) {
    Hutao.World.showHelp(player,
      [`${setting.default.data.commands.normalPrefix}name-tag state §d<Boolean>`, Hutao.Player.getLanguage(player).commandState],
    )
  } else {
    ...
    if (command[2] == "true") {
      config.data.nameTag.state = true
    } else if (command[2] == "false") {
      config.data.nameTag.state = false
    } else {
