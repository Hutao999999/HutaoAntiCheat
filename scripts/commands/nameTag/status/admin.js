...
export const admin = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`status admin §dset`, Hutao.Player.getLanguage(player).commandNameTagStatusAdmin],
      [`status admin §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`admin set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagStatusAdmin]
      )
    } else {
      ...
      config.data.nameTag.format.status.admin = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).admin} §7▶ §r${setting.default.data.nameTag.format.status.admin}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
