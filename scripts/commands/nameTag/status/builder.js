...
export const builder = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`status builder §dset`, Hutao.Player.getLanguage(player).commandNameTagStatusBuilder],
      [`status builder §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`builder set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagStatusBuilder]
      )
    } else {
      ...
      config.data.nameTag.format.status.builder = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).builder} §7▶ §r${setting.default.data.nameTag.format.status.builder}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
