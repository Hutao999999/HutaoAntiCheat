...
export const member = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`status member §dset`, Hutao.Player.getLanguage(player).commandNameTagStatusMember],
      [`status member §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`member set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagStatusMember]
      )
    } else {
      ...
      config.data.nameTag.format.status.member = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).member} §7▶ §r${setting.default.data.nameTag.format.status.member}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
