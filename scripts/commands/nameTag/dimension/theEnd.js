...
export const theEnd = (player, command) => {
  if (command[4] == undefined) {
    Hutao.World.showHelp(player,
      [`dimension the-end §dset`, Hutao.Player.getLanguage(player).commandNameTagDimensionTheEnd],
      [`dimension the-end §dview`, Hutao.Player.getLanguage(player).commandView]
    )
  } else if (command[4] == "set") {
    if (command[5] == undefined) {
      Hutao.World.showHelp(player,
        [`the-end set §d<string>`, Hutao.Player.getLanguage(player).commandNameTagDimensionTheEnd]
      )
    } else {
      ...
      config.data.nameTag.format.dimension.theEnd = Hutao.World.getNormalCommand(player, 5);
    ...
    Hutao.World.log([
      ...
      ``,
      `§e${Hutao.Player.getLanguage(player).theEnd} §7▶ §r${setting.default.data.nameTag.format.dimension.theEnd}`,
      `§7=========================`
    ].join("\n§r"), player)
  } else {
